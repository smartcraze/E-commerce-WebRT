...
const Profile = () => {
  ...
  const [phoneNumber, setPhoneNumber] = useState(
    localStorage.getItem("phoneNumber") || user.phone
  );
  const [address, setAddress] = useState(
    localStorage.getItem("address") || user.address
  );
  const [postalCode, setPostalCode] = useState(
    localStorage.getItem("postalCode") || user.postalCode
  );
  ...
  const handlePhoneNumberChange = (e) => {
    ...
    if (/^\d{0,10}$/.test(input)) {
      ...
      setPhoneNumberError(
        "Phone number should be numeric and maximum 10 digits."
      );
    }
  };
  ...
  return (
    <>
      <div className={`bg-gray-100 min-h-[10vh] ${background}`}>
